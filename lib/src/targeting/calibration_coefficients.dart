...
/// 
/// These camera coefficients are loaded from a json file in the opencv calibration
/// format
class CalibrationCoefficients {
  ...
  /// Creates the camera calibration coefficients from
  CalibrationCoefficients.fromJson({required Json json}) {
    if (json["camera_matrix"] is Map) {
      _initIntrinsics(
        (json["camera_matrix"]["data"] as List<Object?>).cast(),
      );
    } else if (json["camera_matrix"] is List) {
      _initIntrinsics(
        (json["camera_matrix"] as List<Object?>).cast(),
      );
    }
  ...
  void _initIntrinsics(List<num> intrinsicsMatrix) {
    intrinsics = Mat.fromList(3, 3, MatType.CV_64FC1, intrinsicsMatrix.map((e) => e.toDouble()).toList());
  }
