...
class FrameProperties {
  ...
  /// Constructor for frame properties from calibration data
  /// 
  /// Calculates the fov and focal length from the camera matrix
  FrameProperties.fromCalibrationData({
    ...
    horizontalFoV = 2 * atan(captureWidth / (2 * calibrationData!.intrinsics!.at(0, 0))) * (180 / pi);
    verticalFoV = 2 * atan(captureHeight / (2 * calibrationData!.intrinsics!.at(1, 1))) * (180 / pi);
  ...
  FrameProperties.fromDiagonalFoV({
    ...
    centerX = (captureWidth / 2.0) - 0.5;
    centerY = (captureHeight / 2.0) - 0.5;
    final (:horizontal, :vertical, :focal) = calculateHorizontalVerticalFoV(diagonalFoV, captureWidth, captureHeight);
  ...
  ({double diagonal, double focal}) calculateDiagonalFoV({
    ...
    final diagonalFoV = atan(tan(horizontalFoVRad / 2) * (aspectDiag / captureWidth)) * 2 * (180 / pi);
